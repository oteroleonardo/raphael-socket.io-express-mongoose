...
files.forEach(function (file) {
    ...
    console.log("Adding route: " + filePath);
    route.init(app);
});
...
io.sockets.on('connection', function (socket) {
    ...
    socket.get('nickname', function (err, nickname) {
      ...
      if('Opportunity'==nickname){
        if('on_location' == data.message_type){
          ...
          if(i<6){
            socket.emit('new_path', 
              { path: ('M' + data.x + ',' + data.y + instruction[i].path), ETA: instruction[i].ETA});
            i++;        
          }
        }
  ...
  setTimeout(function () {  
    socket.send('Warn: Mars Rover link wait exceded the 10 seconds!'); 
  }, 10000); 
